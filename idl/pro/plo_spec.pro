...
if keyword_set(print) then begin
   if keyword_set(spec_file) then openw,lun, specfile,/get_lun else openw,lun,'spec.data',/get_lun
endif
...
;      phas=make_array(1,/float,/nozero) 
      ...
      for i=0, nbands-1 do begin
        ...
        if keyword_set(normalize) then begin
           ...
           if keyword_set(print) then begin
              printf,lun,rec,' ',sb,' ',band,' ',npts
              for iprint=0,npts-1 do begin
                 printf,lun,x[iprint],amp[iprint],pha[iprint],format='(f16.7,2x,E13.4,2x,f8.2)'
              endfor
           endif       
        endif else begin
           dat_get_rows,cmp,amp,pha,x,wts,first,npts,x_var,0,/list, $
        ...
        endelse
        if distinct_ibands[i] gt 48 and keyword_set(wntrim) then begin
            if x_var eq 'channel' then begin
                x=x+first_chs(ib)
                ntrim = npts[0] gt 2*wntrim ? wntrim : ntrim
            endif else begin
                ntrim = (npts[0] gt 2*floor(wntrim/abs(sp[psl[0]].fres)) ? floor(wntrim/abs(sp[psl[0]].fres)):ntrim)
            endelse
        endif
...
;          'source,pos'        : frame=source+' '+pos
          ...
          'blcd,sb,band'      : frame=blcd+' '+strupcase(sb)+' '+strupcase(band)
