pro readtsys_eng, dir=dir, replace=replace
   ...
   for i=0L,nint-1L do begin
      ...
      for j=0,nants-2 do begin
         for k=j+1,nants-1 do begin
            loc=where(bl[pbl[tmp_idx]].itel1 eq ants[j] and bl[pbl[tmp_idx]].itel2 eq ants[k] and bl[pbl[tmp_idx]].irec le 1, count)
            if count gt 0 then begin
            ...
            loc=where(bl[pbl[tmp_idx]].itel1 eq ants[j] and bl[pbl[tmp_idx]].itel2 eq ants[k] and bl[pbl[tmp_idx]].irec gt 1, count)
            if count gt 0 then begin
               jj=where(tmp_itel eq ants[j])
               tmpj=tmp_tsys2[jj]
               ;tmp[0,*]=tmpj[0,*]
