function pass_sideband,pas_souids,x_var,y_vars,dch_smooth,frame_vars, $
...
for js=0,n_elements(distinct_pas_souids)-1 do begin
  ...
  for i=0,ncombinations-1 do begin
    ...
    blcd=codes(0) & sb=codes[2] 
    frame=blcd+' '+strupcase(sb)
    frames=[frames,frame]

    if (tel_bsl eq 'telescope') then begin
      tmp=bl(pbl).itel1
      cal_itel1[i]=tmp[uniq(tmp)]
      tmp=bl(pbl).itel2
      cal_itel2[i]=tmp[uniq(tmp)]
    endif
    ;
...
if not e.java then begin
   if pindx eq 1 then pl[plid].plot_interact=plo_control(plid,pindex)
