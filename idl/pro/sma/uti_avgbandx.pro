pro uti_avgbandx, all=all, band=band, old=old, swmch1=swmch1, swmch2=swmch2, zeroflag=zeroflag, excludefreq=excludefreq
;yes
...
;   endif
   ...
   for i=0,nlines-1 do print,'   ',excludefreq[2*i], ' to ',excludefreq[2*i+1],' GHz ...'
endif
...
for i=0L, count-1L do begin
   ...
   if keyword_set(excludefreq) then begin
      ...
      for j=0,nb-1 do begin
         ...
         chunkfreq[2*j]=sp[psl[icont[i]+j+1]].fsky-(npts[j]/2-skip1[j])*abs(chunkfres[j])
         chunkfreq[2*j+1]=sp[psl[icont[i]+j+1]].fsky+(npts[j]/2-skip2[j])*abs(chunkfres[j])
      endfor
;      print,chunkfreq
...
;                  excludechan[inb,iexclude+1]=128+fix((excludefreq[m+1]-sp[psl[icont[i]+inb+1]].fsky)/chunkfres[inb])                  
                  tmp1=128+fix((excludefreq[m]-sp[psl[icont[i]+inb+1]].fsky)/chunkfres[inb])
                  tmp2=128+fix((excludefreq[m+1]-sp[psl[icont[i]+inb+1]].fsky)/chunkfres[inb])
...
;                  print,excludechan[inb,iexclude+1]
                  ...
                  if (loc[m+1]-loc[m]) ge 2 then begin	
