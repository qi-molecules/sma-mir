function pass2, pas_souids,x_var,y_vars,dch_smooth,frame_vars, $
...
pt_first=make_array(nsbs,/long,value=-1L)
pt_npts=make_array(nsbs,/long,value=-1L)
frames=make_array(nsbs,/string,value='')
...
;pt_first2=make_array(nrecs*nsbs*nbands,/long,value=-1L)
;pt_npts2=make_array(nrecs*nsbs*nbands,/long,value=-1L)
npt=0L
...
;            endif
;            if pt_first2[index] eq -1L then begin
;               pt_first2[index]=1+total(pt_npts2 > 0)
;               npt=npt+npts[0]
;               xs=[xs,x]
;               cmps=[cmps,cmp]
;               amps=[amps,amp]
;               phas=[phas,pha]
;            endif else begin
;               ; not working for second passband source.
;               cmps[pt_first2[index]:pt_first2[index]+pt_npts2[index]-1L]= $
;                 cmps[pt_first2[index]:pt_first2[index]+pt_npts2[index]-1L]+cmp
;            endelse
;            if pt_npts2[index] eq -1L then pt_npts2[index]=npts[0]
         ...
         frames[k]=strupcase(sb)
         pt_first[k]=1+total(pt_npts>0)
         if k gt 0 then pt_npts[k]=npt-total(pt_npts)-1L else pt_npts[k]=npt
         jump1:
      endfor                    ; sbs
   endfor                       ; recs
;endfor                          ; baselines
...
result=pass_fit(tel_bsl,funct,dch_smooth,x_var,y_vars,frames, $
          ...
          ntrim_max=ntrim_max, npoly=npoly, delay=delay, icursor=icursor)
