function gain_sideband,gai_souids,gai_fluxes_3mm,gai_fluxes_1mm,tel_bsl, $
         ...
         polar=polar, sideband=sideband, yfs=yfs, xfs=xfs, orig_flux=orig_flux
;
...
;
  ...
  if keyword_set(sideband) then begin
     if sideband eq 'u' or sideband eq 'l' then begin
        result=dat_list(s_l,'"sb" eq "'+sideband+'"',/no_notify)
     endif else begin
        print,'*** WRONG SIDEBAND NAME, USE u or l only ***'
        return,-1
     endelse
  endif
...
;    eq 'callisto')  then begin
    ...
    if ((non_point gt 0.) and (in[pil[kkk[0]]].size gt 0.)) then begin
      ...
      if sp[pil[0]].nch eq 1 then begin
         bw     = sp[psl].fres
         freq   = sp[psl].fsky
      endif else begin
         bw     = bl[pbl].bwave
...
;
    ...
    if (not keyword_set(preavg)) then begin
      print,"Number of rec, sb, band, int, antenna:", $
                      nofrec,nofsb,nofband,nofint,noftel
  
      gain_size = nofrec * nofsb * nofband * nofint * noftel
    endif else begin
      print,"Number of rec, sb, band, group, antenna:", $
...
;
    ...
    for fcount=0,(nframes -1) do begin
      ...
      if (nbsl ne ((ntel)*(ntel-1)/2)) then begin
         print, 'WARMING: mismatch between number of ANTENNAS and number of BASELINES at '+ $
      ...
      if (where(listtel eq refant) eq -1) then begin
         print, 'WARMING: missing data from the reference antenna at ' + $
                          distinct_frames[fcount]+'.'
         print, '         all records at this rx/sb/band/int are flagged in solution derivation !!!'
         weight = -1.
      endif
...
if pindx eq 1 then begin
   result=plo_page(plid,gindex)
   gai[gindex].color_index=color_index+150.
   gai[gindex].nsub=n_elements(gai[gindex].y_var)-1
   result=plo_over_page(plid,gindex)
endif
gai[gindex].nsub=n_elements(gai[gindex].y_var)

if not e.java then begin
   if pindx eq 1 then pl[plid].plot_interact=plo_control(plid,gindex)

print, "EXITING FROM THE INTERACTIVE MODE"
 
   result=gain_store(tel_bsl,x_var,y_vars,plid,gindex)
   plo_plid_rel,plid
endif
