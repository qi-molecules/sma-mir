pro apply_we,dir=dir,sign=sign,smoothing=smoothing, antennas=antennas

common global
common data_set

   if not keyword_set(sign) then sign=1 
   if not keyword_set(smoothing) then smoothing=5
...
; load in we structure
   ...
   if not keyword_set(antennas) then begin 
...
; calculate the phase change according to Mark Gurwell's log 2720. 
      ...
      for i=0L, nint-1L do begin       
         ...
         tempfrac=antrefrac[*,ii[i]-1]
         resistant_mean,tempfrac,3,robustmean,sig,num
	 
;         drefrac=yfs[ii[i]]-we[ii[i]].refrac[0]
         drefrac=robustmean-we[ii[i]-1].refrac[0]
         for ical = 0L, nbcals-1L do begin
            js = where(distinct_bcals[ical] eq bcals,njs)
            if (njs gt 0L) then begin
               js_cont=where(abs(sp(psl(js)).nch) lt 10,njs_cont)
               for j=0L, njs-1L do begin
                  jsj=js[j]
                  lambda=!cvel/sp[psl[jsj]].fsky/1e9
                  dL=sign*drefrac*1e-6*bl[pbl[jsj]].blu/sin(in[pil[jsj]].el*!DPI/180.d) ; blu is in unit of meter
                  phase=dL/lambda
