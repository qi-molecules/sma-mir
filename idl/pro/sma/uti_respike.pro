...
;         blcd=strcompress(string(ibsl[0]),/remove)
         ...
         n=where( (in[pil].int eq iscan[ichan[j]]) and (bl[pbl].irec eq rx[ichan[j]]) and (bl[pbl].isb eq isb[ichan[j]]) and (bl[pbl].iblcd eq ibsl[0]) and (sp[psl].iband eq band[ichan[j]]), count2)
...
;         if (result gt 0) and finite(f1[ichan[j]]) then begin
         if (count2 gt 0) and finite(f1[ichan[j]]) then begin 
;            ptr=pcl+channel[i]-1L
            ptr=pcl[n]+channel[i]-1L
;            print,'ptr is ',ptr
            ch[ptr]=complex(f1[ichan[j]],f2[ichan[j]])
            if keyword_set(verbose) then print,'Spike restored for scan ', iscan[ichan[j]], ', receiver ',c.rec[rx[ichan[j]]], ', sideband ',c.sb[isb[ichan[j]]], ', baseline ',c.blcd[ibsl[0]], ', chunk ',c.band[band[ichan[j]]], ', channel ',channel[i]
         endif
      endfor
   endif
endfor
