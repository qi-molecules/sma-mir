function dat_list ,s_l,selection,reset=reset,save=save,restore=restore, $
                   no_notify=no_notify
;
; Creates pointer arrays to list the data set with restrictions.
; pil,pbl,psl,pcl,prl are the restricted list pointer arrays. The 
...
;
; Note : There are serial routines dat_filter.pro which produces the
;        pointer list to be used globally and dat_select.pro. 
;        The filter and select pointers should be created first since 
;        the lists built here start from those produced by dat_filter.
;
; parameters : s_l -- save structure for lists (must always include)
...
;
; eg.: result=dat_list(s_l,/reset) ; simply reset and pass all data 
; eg.: result=dat_list(s_l,'"band" eq "c1"',/reset) ; pick out just c1 data
; eg.: result=dat_list(s_l,'"band" like "c"',/reset) ; pick out all c1 & c2 data
; eg.: result=dat_list(s_l,'"blcd" like "1"',/reset) ; pick out all bsl w/ tel 1
; eg.: result=dat_list(s_l,'"band" in ["c1","c2"]',/reset) ; pick out c1 & c2
; compound example: to pick out bsl 1-2, 1-3, & 1-6 could use :
; eg.: result=dat_list(s_l,'"blcd" in ["1-2","1-3"] or "blcd" eq "1-6"',/reset) 
; eg.; result=dat_list(s_l,'"blcd" in ["1-2","1-3"]',/reset)
; eg.: result=dat_list(s_l,'"blcd" in ["1-2","1-3"] or "blcd" in ["4-5"]',/reset)
;
common global
common data_set
if keyword_set(reset) then begin
  ...
  pcl=pcs
  prl=prs
  s_l={a:0} ; remove s_l save structure if not needed
endif
if keyword_set(save) then begin
  s_l={pil:pil,pbl:pbl,psl:psl,pcl:pcl,prl:prl}
endif
if keyword_set(restore) then begin
  if n_tags(s_l) le 1 then begin
    print,'no save structure or lists, can not restore !'
    return,-1
  endif
  pil=s_l.pil & pbl=s_l.pbl & psl=s_l.psl & pcl=s_l.pcl & prl=s_l.prl
endif
  nrows=n_elements(psl)
  if max(psl) eq -1 then nrows=0
  if n_params() le 1 then return,nrows
;
...
;
if not dat_var_info(var,s,stag,tag,tagno,pointer_str,'l',label,format, $
...
;
  ...
  case 1 of
    ...
    print,condition,' not recognized by dat_list routine'
    return,-1
    end
  endcase
...
if e.prog_help then begin
 ...
 print,'    pil, pbl, psl, pcl, and prl should now be used in place of '$
      ,'    pis, pbs, pss, pcs, and prs.'$
      ,'    pcl is pointer array from sp rows to the first data pt in ch'$
      ,'    prl is pointer array from sp rows to the first record of header'  $
      ,'    ... data in re'
endif
return,nrows
end
