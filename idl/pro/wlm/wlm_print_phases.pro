...
;
; PURPOSE
;      Prints phases before and after WLM corrections for the entire track.
;
; INPUTS
;      phase_corr : Phase corrections (in degrees) applied to the data
;      cal_cmp    : Phase corrections in complex data format
;      coherence  : If set, only coherence corrections are applied
;                   Used here only to set output file name.
;
; OUTPUT
;      -1   Failed.
;       1   Successful
...
pro wlm_print_phases,phase_corr,cal_cmp,coherence=coherence
   ; Common blocks
     common global
     common data_set
     common wlm
   ...
   ; Symbol styles
     pstyle = [16, 6, 5, 2, 8, 9,replicate(16,n_elements(c.source))]
