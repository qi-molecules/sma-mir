function map_clean_driver,plid,image,beam,udata,vdata,pixel_size,$
   ...
   ; Find the peak pixel position in the map
     nx = n_elements(image[0,*])
     ny = n_elements(image[*,0])
     peak = max(image,imax)
     iypeak = fix(imax/nx)
     ixpeak = imax - nx*iypeak

   ; Clean the map
     map_clean,image,beam,ixpeak,iypeak,1.0,cleaned_image,resid_image
print,min(image),min(cleaned_image),min(resid_image),min(cleaned_image+resid_image)
print,max(image),max(cleaned_image),max(resid_image),max(cleaned_image+resid_image)
; stop
;    cleaned_image = cleaned_image + resid_image
   ...
   ; Plot data
     ...
     while iframe le nframes-1 do begin
       begin_page:
         ...
         contour,cleaned_image/max([abs(cleaned_image)]),x,-y,levels=levels, $
