FUNCTION  UTI_FIT_ROBUST,XIN,YIN,YFIT,SIG,SS, NUMIT=THIS_MANY, BISECT=TYPE, $
                         Bisquare_Limit=nsig, $
...
;
; CALLING SEQUENCE:
;	COEFF = UTI_FIT_ROBUST( X, Y, YFIT, SIG, COEF_SIG, [ BISECT = ,
...
; SUBROUTINE CALLS:
;	UTI_MED, to calculate the median
;	UTI_CHECKFIT
;	UTI_SIGMA, to calculate a robust analog to the std. deviation
;
; PROCEDURE:
...
;      12/98 --changed FLTARR to DBLARR - JMC
;-
...
IF (XRANGE LT EPS) OR (XRANGE/AVEX LT DEL) THEN BEGIN
   PRINT,'UTI_FIT_ROBUST: Independent variables the same. No fit possible.'
   RETURN,0.
ENDIF 
...
;  We divide the data into 2 groups and fit a line to their X and Y medians.
   ...
   NHALF=N/2-1
   X1=UTI_MED(U(0:NHALF)) & X2=UTI_MED(U(NHALF+1:N-1))
   Y1=UTI_MED(V(0:NHALF)) & Y2=UTI_MED(V(NHALF+1:N-1))
...
;     The X medians are too close. Select the end-points instead.
   ...
   YFIT=CC(0)+CC(1)*X
   ISTAT=UTI_CHECKFIT(YP,YFIT,EPS,DEL,SIG,FRACDEV,NGOOD,W,S,nsig=nsig)
...
IF (LSQ EQ 1) OR (N LT 6) THEN BEGIN  ; Try a least-squares fit
   ...
   IF ABS(D) LT EPS THEN BEGIN
      PRINT,'UTI_FIT_ROBUST: No fit possible.'
      RETURN,0.
   ENDIF 
...
;     Get the X vs Y line.
      ...
      D=SYY-SY*SY
      IF ABS(D) LT EPS THEN BEGIN
         PRINT,'UTI_LINEFIT: No fit possible.'
         RETURN,0.
      ENDIF
...
;     coordinates.
      ...
      YFIT=DBLARR(N) & YFIT(*)=0. ; to fool UTI_CHECKFIT
      SS=YP
   ENDIF ELSE BEGIN
...
IF NGOOD LT 2 THEN BEGIN
   PRINT,'UTI_LINEFIT: Data Dangerously Weird. Fit Questionable.'
   BADFIT=1
   GOTO,AFTERFIT
ENDIF
...
;    Now calculate the equation of the bisector of the 2 lines:
  ...
  IF ISTAT EQ 0 THEN GOTO,AFTERFIT
  IF NGOOD LT 2 THEN BEGIN
     PRINT,'UTI_FIT_ROBUST: Data Dangerously Weird. Fit Questionable.'
     BADFIT=1
     GOTO,AFTERFIT
  ENDIF
...
IF (N_PARAMS(0) GT 3) AND (SIG GT EPS) AND (NGOOD GT 2) THEN BEGIN
   ...
   ; Check to see if this is negative or not
   arg = MAX([0.0,SS(0)^2+X0*SS(1)^2])
   SS(0)=SQRT(arg)
ENDIF
