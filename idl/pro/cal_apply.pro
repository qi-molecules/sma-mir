function cal_apply,gain=gain,passband=passband,wlm=wlm,mklog=mklog, $
...
;
                  FOR k=0,n_elements(js)-1L DO BEGIN
                     is=lindgen(sp[psl[js[k]]].nch,sp[psl[js[k]]].nrec)/sp[psl[js[k]]].nrec
                     ch[pcl[js[k]]:pcl_end[js[k]]]= $
                        ch[pcl[js[k]]:pcl_end[js[k]]]/passband_cmp[is]
                  ENDFOR
               ENDIF
