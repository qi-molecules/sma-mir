function reduce_plo_spec,x_var,y_vars,frame_vars,color_vars, $
...
result=dat_list(s_l,'"band" not like "c"',/reset,/no)
...
result=dat_list(s_l,'"band" not like "c" and "source" like "'+ $
                 distinct_sources(js)+'"',/reset,/no)
...
if strpos(frame_vars,'blcd',0) ne -1 or strpos(color_vars,'blcd',0) ne -1 then $
    blave=0
if not blave then bls=c.blcd(bl(pbl).iblcd)
...
if blave then combinations=recs+' '+sbs+' '+bands $
   else combinations=bls+' '+recs+' '+sbs+' '+bands
distinct_combinations=uti_distinct(combinations,ncombinations,/many_repeat)
; If the number of telescopes is less then 6, then add in the blank fields
if n_elements(c.tel1) lt 5 then begin
   subcom = uti_distinct(recs+' '+sbs+' '+bands)
   for i=1,6 do begin
   for j=1,6 do begin
      if (j gt i) then begin
        tbls = strcompress(string(i) + '-' + string(j),/remove)
        k = where(tbls eq bls,nk)
        if (nk eq 0) then $
           distinct_combinations = [distinct_combinations,tbls+" "+subcom]
      endif
   endfor
   endfor
   distinct_combinations = uti_distinct(distinct_combinations,ncombinations)
endif
...
;
  if not blave then begin
    ...
    nrows=dat_list(s_l,'"blcd" eq "'+codes(0)+ $
        '" and "wt" gt "0" and "rec" eq "'+codes(1)+'"and "sb" eq "'+codes(2)+ $
        '" and "band" eq "'+codes(3)+'"',/no)
    blcd=codes[0] & rec=codes[1] & sb=codes[2] & band=codes[3]
  endif
  if blave and result gt 0 then begin
    result=dat_comb_sep(distinct_combinations[i],['rec','sb','band'], $
                      codes,icodes,n_components)
    nrows=dat_list(s_l,'"rec" eq "'+codes(0)+'"and "sb" eq "'+ $
        codes(1)+'" and "wt" gt "0" and "band" eq "'+codes(2)+'"',/no_notify)
    rec=codes[0] & sb=codes[1] & band=codes[2]
  endif
...
;    print,'could not average ',distinct_combinations(i)
;    distinct_combinations(i)='bad'
;    goto, jump
;  endif
...
;
   jj = where(cmp ne 0.0,njj)
   if (nrows eq 0 or njj eq 0) then begin
      npts = 0
      pt_first=[pt_first,npt]
      pt_npts=[pt_npts,npts]
      ; cmps=[cmps,0.0]
      ; amps=[amps,amp[ntrim:n_elements(cmp)-ntrim-1]]
      ; phas=[phas,pha[ntrim:n_elements(cmp)-ntrim-1]]
      ; xs=[xs,x[ntrim:n_elements(cmp)-ntrim-1]]
      npt=npt+npts[0]
   endif else begin 
      ntrim = keyword_set(ntrim_max)? (npts[0] gt 2*ntrim_max ? ntrim_max : 0) : 0
      ntrim = 0
      pt_first=[pt_first,npt]
      pt_npts=[pt_npts,npts[0]-2*ntrim]
      cmps=[cmps,cmp[ntrim:n_elements(cmp)-ntrim-1]]
      amps=[amps,amp[ntrim:n_elements(cmp)-ntrim-1]]
      phas=[phas,pha[ntrim:n_elements(cmp)-ntrim-1]]
      xs=[xs,x[ntrim:n_elements(cmp)-ntrim-1]]
      npt=npt+npts[0]-2*ntrim
   endelse
;
...
;
distinct_combinations=uti_distinct(where(distinct_combinations ne 'bad'),/many_repeat)
...
phas=phas[1:npt]
xs=xs[1:npt]
pt_first=pt_first[1:ncombinations]
pt_npts=pt_npts[1:ncombinations]
...
;    if !d.name ne 'PS' then result=plo_zbuf(java=e.java)
